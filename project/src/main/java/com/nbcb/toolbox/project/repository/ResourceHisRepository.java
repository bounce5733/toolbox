...
public interface ResourceHisRepository extends JpaRepository<ResourceHis, Integer> {
    ...
    @Query(value = "SELECT sp.dept, rs.start_date, rs.end_date, p.domain, s.type PERSONNEL_TYPE FROM resource_his rs" +
            ...
            " AND (p.domain in :domains OR :domains IS NULL)", nativeQuery = true)
    List<Map<String, Object>> findByCustomParams(@Param("dept") String dept, @Param("startDate") String startDate,
                                                 @Param("endDate") String endDate, @Param("domains") List<String> domains);
}
